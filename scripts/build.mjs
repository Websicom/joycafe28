...
import { getDisplayHours, RESERVATION_CONFIG } from '../shared/reservations.js';
...
const [site, menu, faqs, announcements] = await Promise.all([
  readJson('site.json'), readJson('menu.json'), readJson('faqs.json'), readJson('announcements.json')
]);
site.hours = getDisplayHours();
...
const replacements = {
  ...
  PARTY_OPTIONS: Array.from({ length: RESERVATION_CONFIG.partySize.max - RESERVATION_CONFIG.partySize.min + 1 }, (_, index) => index + RESERVATION_CONFIG.partySize.min).map((size) => `<option value="${size}">${size} ${size === 1 ? 'person' : 'people'}</option>`).join(''),
  ...
  EVENING_MENU_TITLE: escapeHtml(menu.evening.title), EVENING_MENU_AVAILABILITY: escapeHtml(menu.evening.availability), EVENING_MENU_INTRO: escapeHtml(menu.evening.intro), EVENING_MENU_ITEMS: renderMenu(menu.evening.items), EVENING_ALLERGY_NOTE: escapeHtml(menu.evening.allergyNote),
...
const iconLinks = `<link rel="icon" href="/favicon.ico" sizes="any"><link rel="icon" href="/assets/brand/joy-favicon.png" type="image/png"><link rel="icon" href="/assets/brand/joy-favicon.webp" type="image/webp"><link rel="apple-touch-icon" href="/assets/brand/apple-touch-icon.png" sizes="180x180"><link rel="apple-touch-icon" href="/assets/brand/apple-touch-icon-152x152.png" sizes="152x152"><link rel="apple-touch-icon" href="/assets/brand/apple-touch-icon-120x120.png" sizes="120x120">`;
const legalPage = ({ title, description, slug, body }) => `<!doctype html><html lang="en-GB"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(title)} | Joy Cafe Melbourn</title><meta name="description" content="${escapeHtml(description)}"><link rel="canonical" href="${site.domain}/${slug}"><meta name="robots" content="index,follow"><meta name="theme-color" content="#fbfaf6">${iconLinks}<link rel="stylesheet" href="/assets/css/styles.css"><script type="module" src="/assets/js/navigation.js"></script></head><body class="legal-page" data-theme="day"><a class="skip-link" href="#main">Skip to content</a><header class="site-header simple-header"><a class="brand" href="/" aria-label="Joy Cafe home"><img src="/assets/brand/joy-main-logo.svg" alt="" width="900" height="820"></a><nav class="primary-nav" aria-label="Legal page navigation"><a href="/">Home</a><a href="/#day-menu">Menus</a><a href="/#visit">Visit</a></nav><a class="button header-book" href="/#book">Book a Table</a></header><main id="main"><div class="legal-header"><div><p class="eyebrow">Joy Cafe & Wine Bar</p><h1>${escapeHtml(title)}</h1></div></div><article class="legal-content">${body}</article></main>${legalFooter}</body></html>`;

const privacyBody = `<p><strong>Last updated: 5 September 2026</strong></p><p>This policy explains how ${escapeHtml(site.company)} handles personal information when you visit our website or contact us.</p><h2>Who we are</h2><p>${escapeHtml(site.company)} (company number ${escapeHtml(site.companyNumber)}) operates Joy Cafe & Wine Bar at ${escapeHtml(site.address.street)}, ${escapeHtml(site.address.locality)}, ${escapeHtml(site.address.town)}, ${escapeHtml(site.address.postcode)}. Contact us at <a href="mailto:${escapeHtml(site.email)}">${escapeHtml(site.email)}</a>.</p><h2>Information you choose to share</h2><p>When you send a reservation request, we collect the contact and visit details entered in the form so our team can respond and manage your request. A request is not an automatic booking confirmation.</p><h2>Form security and delivery</h2><p>Cloudflare processes the form submission, verifies the security challenge and sends the request to our fixed business email address. We do not send an automatic customer email. Security logs are limited to technical outcomes and do not intentionally include the form contents.</p><h2>Technical information</h2><p>Our hosting provider may process ordinary server logs, such as IP address, browser type, requested page and time of access, to operate and secure the website. These records are retained only as needed for security and service operation.</p><h2>Sharing and retention</h2><p>We do not sell personal information. We share it only with service providers where needed to run the website or manage your request, or where required by law. We keep correspondence and reservation information only for as long as reasonably needed for the purpose it was collected and our legal obligations.</p><h2>Your rights</h2><p>UK data protection law may give you rights to access, correct, erase or restrict the use of your personal information, and to object or complain. Contact us first at <a href="mailto:${escapeHtml(site.email)}">${escapeHtml(site.email)}</a>. You may also contact the Information Commissioner's Office.</p><h2>External links</h2><p>Links to Instagram, map services and other websites take you to services with their own privacy policies.</p>`;
const cookieBody = `<p><strong>Last updated: 5 September 2026</strong></p><p>Joy Cafe's website does not intentionally use analytics cookies, advertising cookies or tracking pixels.</p><h2>Day and night preference</h2><p>If you manually choose the Day or Night appearance, that choice is stored in your browser's <code>sessionStorage</code>. It lasts only for the current browsing session and is not a cookie.</p><h2>Announcements</h2><p>The opening announcement is checked against the current date. We do not use cookies or local storage to remember it.</p><h2>External services</h2><p>The reservation form uses Cloudflare Turnstile to distinguish people from automated submissions. We do not load a map until you choose “Show Map”. If you visit Instagram, open the map or follow another external link, that provider may use cookies under its own policy. You can control cookies through your browser settings.</p><h2>Changes</h2><p>If we introduce analytics or another feature that requires non-essential cookies, we will update this policy and provide an appropriate choice before those cookies are set.</p><h2>Contact</h2><p>Questions? Email <a href="mailto:${escapeHtml(site.email)}">${escapeHtml(site.email)}</a>.</p>`;
const termsBody = `<p><strong>Last updated: 5 September 2026</strong></p><p>These terms apply to your use of the Joy Cafe & Wine Bar website.</p><h2>Website information</h2><p>We aim to keep information accurate and useful, but menus, prices, seasonal dishes and opening hours may change. Please contact us if you need to confirm something before travelling.</p><h2>Reservation requests</h2><p>The website sends your preferred date, time and party details to the Joy team. Sending the form does not guarantee availability or confirm a booking. If there are any issues, our team will contact you using the details supplied.</p><h2>Allergies</h2><p>Please tell us about allergies or dietary requirements when requesting a reservation and again when you arrive. We will always do our best to accommodate you, but the website is not a substitute for discussing your needs directly with our team.</p><h2>External links</h2><p>Links to other websites are provided for convenience. We do not control their content, availability or privacy practices.</p><h2>Intellectual property</h2><p>The Joy name, logo, website design, copy and original photography are owned by or licensed to ${escapeHtml(site.company)}. Please do not reproduce them without permission.</p><h2>Availability and liability</h2><p>We may update, suspend or withdraw parts of the website. Nothing in these terms excludes rights or responsibilities that cannot lawfully be excluded, including your statutory consumer rights.</p><h2>Law</h2><p>These terms are governed by English law, and disputes are subject to the jurisdiction of the courts of England and Wales.</p><h2>Contact</h2><p>${escapeHtml(site.company)}, ${escapeHtml(site.address.street)}, ${escapeHtml(site.address.locality)}, ${escapeHtml(site.address.town)}, ${escapeHtml(site.address.postcode)}. Email <a href="mailto:${escapeHtml(site.email)}">${escapeHtml(site.email)}</a>.</p>`;

await Promise.all([
  writeFile(path.join(root, 'privacy-policy.html'), legalPage({ title: 'Privacy Policy', description: 'How Joy Cafe Ltd handles personal information on the Joy Cafe website.', slug: 'privacy-policy.html', body: privacyBody.replace('Cloudflare processes the form submission, verifies the security challenge and sends the request to our fixed business email address.', 'Cloudflare validates the form submission and applies spam protection. Google Apps Script delivers the request to our fixed business email address.').replace('Last updated: 5 September 2026', 'Last updated: 24 September 2026') })),
  writeFile(path.join(root, 'cookie-policy.html'), legalPage({ title: 'Cookie Policy', description: 'Cookies and browser storage used by the Joy Cafe website.', slug: 'cookie-policy.html', body: cookieBody.replace('The reservation form uses Cloudflare Turnstile to distinguish people from automated submissions. ', '').replace('Last updated: 5 September 2026', 'Last updated: 24 September 2026') })),
...
await writeFile(path.join(root, 'site.webmanifest'), JSON.stringify({ name: site.name, short_name: 'Joy Cafe', description: 'Healthy by day, naughty by night.', start_url: '/', display: 'standalone', background_color: '#fbfaf6', theme_color: '#fbfaf6', icons: [{ src: '/assets/brand/joy-favicon-192.png', sizes: '192x192', type: 'image/png' }, { src: '/assets/brand/joy-favicon-512.png', sizes: '512x512', type: 'image/png' }, { src: '/assets/brand/joy-favicon.webp', sizes: '720x720', type: 'image/webp' }] }, null, 2));

const out = path.join(root, 'dist');
await rm(out, { recursive: true, force: true });
await mkdir(out, { recursive: true });
for (const item of ['index.html', 'privacy-policy.html', 'cookie-policy.html', 'terms.html', '404.html', 'robots.txt', 'sitemap.xml', 'site.webmanifest', 'favicon.ico', 'assets', 'data', 'shared']) {
